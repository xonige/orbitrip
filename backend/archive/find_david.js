...
async function scan() {
    console.log('--- DEEP SCANNING FOR DAVID AND OTHERS ---');
    try {
        const rootFolders = await listFolders('drivers/');
        console.log(`Checking ${rootFolders.length} folders...`);
                    ...
                    if (!sub.id) {
                        ...
                        for (const file of files) {
                            if (file.name.toLowerCase().includes('david') ||
                                ...
                                file.name.toLowerCase().includes('aqua')) {
                                console.log(`MATCH: drivers/${folder.name}/${sub.name}/${file.name}`);
                            }
                        }
                    } else {
                        if (sub.name.toLowerCase().includes('david') ||
                            sub.name.toLowerCase().includes('bmw')) {
                            console.log(`MATCH: drivers/${folder.name}/${sub.name}`);
                        }
                    }
                }
                await sleep(100);
