  ...
  const handlePlan = async () => {
    ...
    try {
        ...
        const driversWithPrice = compatible.map(d => {
            ...
            const MOUNTAIN_COEFF = 1.4; // Updated: 40% increase for mountains
                        ...
                        {matchedDrivers.map(d => (
                            <div key={d.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition bg-white">
                                <div className="flex items-center gap-3 mb-3">
                                    <img src={getOptimizedImageUrl(d.photoUrl)} className="w-10 h-10 rounded-full object-cover" />
                                    <div>
                                ...
                                <div className="flex justify-between items-end">
                                    <div className="text-lg font-black text-blue-600">{d.calculatedPrice} GEL</div>
                                    <button 
                                        onClick={() => {
